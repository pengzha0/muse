...
#             Train
            self.model.train()
            start = time.time()
            tr_loss,tr_loss_speaker,tr_sisnri = self._run_one_epoch(data_loader = self.train_data, state='train')
            ...
            if self.print: print('Train Summary | End of Epoch {0} | Time {1:.2f}s | '
                      'Train Loss {2:.3f}| avg_sisnri {3:.4f}'.format(
                        epoch, time.time() - start, reduced_tr_loss ,tr_sisnri))

            # Validation
            self.model.eval()
            start = time.time()
            with torch.no_grad():
                val_loss, val_loss_speaker,val_sisnri = self._run_one_epoch(data_loader = self.validation_data, state='val')
            ...
            start = time.time()
            with torch.no_grad():
                test_loss,_ ,test_sisnri= self._run_one_epoch(data_loader = self.test_data, state='test')
            ...
            if self.print: print('Test Summary | End of Epoch {0} | Time {1:.2f}s | '
                      'Test Loss {2:.3f}| avg_sisnri {3:.4f}'.format(
                          epoch, time.time() - start, reduced_test_loss,test_sisnri))
            ...
            if self.print:
                ...
                torch.save(checkpoint, "logs/"+ self.args.log_name+"/model_last_dict.pt")
                if reduced_val_loss < self.best_val_loss:
                    ...
                    checkpoint = {'model': self.model.state_dict(),
                                    ...
                                    'best_val_loss': self.best_val_loss,
                                    'val_no_impv': self.val_no_impv}
                    torch.save(checkpoint, "logs/"+ self.args.log_name+"/model_best_dict.pt")
    ...
    def _run_one_epoch(self, data_loader, state):
        ...
        return total_loss / (i+1), total_loss_speaker/ (i+1),torch.mean(avg_sisnri/(i+1)).item()
